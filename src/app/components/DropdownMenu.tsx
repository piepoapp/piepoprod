...
export function DropdownMenu({
  ...
  return (
    <div ref={rootRef} className={`relative ${fullWidth ? "block w-full" : "inline-block"}`}>
      <div onClick={() => setOpen((v) => !v)}>{trigger}</div>
      {open && (
        <div
          className={`absolute z-50 min-w-[180px] bg-white rounded-[8px] border border-[#e4e4e7] shadow-[0px_8px_24px_-4px_rgba(0,0,0,0.12)] p-[4px] animate-in fade-in zoom-in-95 duration-100 ${panelPosition}`}
        >
          {items.map((item, i) => (
